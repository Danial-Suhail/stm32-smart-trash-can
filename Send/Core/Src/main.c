...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
      charToTransmit[0] = 48; // ASCII '0'

      // Update the value based on GPIO states
      if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_3) == GPIO_PIN_SET)
      {
          charToTransmit[0] = 49; // ASCII '1'
      }
      else if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1) == GPIO_PIN_SET)
      {
          charToTransmit[0] = 50; // ASCII '2'
      }
      else if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_5) == GPIO_PIN_SET)
      {
          charToTransmit[0] = 51; // ASCII '3'
      }

      // Transmit the determined value
